...
// Parameters parse_command_line_arguments(int argc, char* argv[]) {
Parameters parse_arguments(const std::vector<std::string>& argv) {
  int argc = argv.size() + 1; 
    ...
    if(argc == 2 &&  std::string(argv[0]) == TOKEN_HELP) {
        print_help();
        exit(EXIT_SUCCESS);
    }

    auto parameters = Parameters(std::string(argv[0]));

    for(auto n = 2; n < argc; n+=2) {

        auto token = std::string(argv[n-1]);
