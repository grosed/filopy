...
//  std::list<std::list<int> > store_to_list(const cobra::Instance& instance, const cobra::Solution& solution)
auto store_to_list(const cobra::Instance& instance, const cobra::Solution& solution)
  {
	...
	sol.push_back(customer);
      }
      routes.push_back(sol);
    }
    auto cost = solution.get_cost();
    return std::make_pair(routes,cost);
      // return routes;
  }
...
// auto filo(int argc, char* argv[]) -> int {
// std::list<std::list<int> > filo(const std::vector<std::string>& args) {
// auto filo(const std::vector<std::string>& args)
auto filo(const std::string& vrpfile)
{
  ...
  auto arg_parser = parse_arguments(args);
    ...
    return store_to_list(instance, best_solution);
...
PYBIND11_MODULE(filopy, m)
{  
  m.doc() = "pybind11 example plugin"; // optional module docstring
  m.def("filo", &filo, "print out and return a string");
}
